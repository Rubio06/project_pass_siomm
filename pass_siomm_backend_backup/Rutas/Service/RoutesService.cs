...
namespace pass_siomm_backend.Rutas.Service
{
    public class RoutesService
    {
        ...
        public async Task<List<RutasPrimarias>> ObtenerRutas()
        {
            ...
            using (SqlCommand cmd = new SqlCommand(SqlQueriesRutas.SP_GET_OBTENER_RUTAS, conn))
            {
...
<<<<<<< HEAD
                    ...
                    var tipoLabor = rutaSec.rutas_terciarias
                        .FirstOrDefault(x => x.nom_ruta_terc == "Tipo_de_Labor");

                    if (nivelIndex >= 0 && tipoLabor != null)
                    {
