...
namespace pass_siomm_backend.Planeamiento.Services.PlaneamientoService
{
    public class AperPeriodoOperativoServices
    {
        ...
        public async Task<List<MaeSemanaAvanceDto>> LeerSeamanAvance(SqlDataReader reader)
        {
            ...
            while (await reader.ReadAsync())
            {
                lista.Add(new MaeSemanaAvanceDto
                {
                    ...
                    fec_ini = reader["fec_ini"] == DBNull.Value ? null : Convert.ToDateTime(reader["fec_ini"]),
                    fec_fin = reader["fec_fin"] == DBNull.Value ? null : Convert.ToDateTime(reader["fec_fin"]),
                    desc_semana = reader["desc_semana"]?.ToString()
                });
            }
        ...
        public async Task<List<string>> labEstandarBloqueo(string anio, string mes, string tipoConsulta)
        {
            var bloqueoListado = new List<string>();

            using var conn = new SqlConnection(_connectionString);
            await using var cmd = new SqlCommand(SqlQueries.SP_LISTA_BLOQUEO, conn);

            cmd.CommandType = CommandType.StoredProcedure;

            cmd.Parameters.Add("@cie_ano", SqlDbType.VarChar, 4).Value = anio;
            cmd.Parameters.Add("@cie_per", SqlDbType.VarChar, 2).Value = mes;
            cmd.Parameters.Add("@tipoConsulta", SqlDbType.VarChar, 50).Value = tipoConsulta;
            ...
            while (await reader.ReadAsync())
            {
                bloqueoListado.Add(reader[0].ToString());
            }

            return bloqueoListado;
        }


        public async Task<List<DatosSemanasDto>> listaEnterosConFecha(string anio, string mes, string tipoConsulta)
        {
            var bloqueoListado = new List<DatosSemanasDto>();

            using var conn = new SqlConnection(_connectionString);
            await using var cmd = new SqlCommand(SqlQueries.SP_LISTA_NUM_ENTEROS, conn);
            ...
            while (await reader.ReadAsync())
            {
                bloqueoListado.Add(new DatosSemanasDto
                {
                    num_semana = Convert.ToInt32(reader["num_semana"]), // o reader[0]
                    fec_fin = Convert.ToDateTime(reader["fec_fin"])    // o reader[1]
                });
            }
