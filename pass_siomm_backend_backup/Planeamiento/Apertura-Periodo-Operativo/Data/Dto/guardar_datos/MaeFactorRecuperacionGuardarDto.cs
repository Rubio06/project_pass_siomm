namespace pass_siomm_backend.Planeamiento.Data.Dto.guardar_datos
{
    public class MaeFactorRecuperacionGuardarDto
    {

        // 🔹 Claves
        public string cod_empresa { get; set; } = string.Empty;
        public string cod_empresa_unidad { get; set; } = string.Empty;
        public string cie_ano { get; set; } = string.Empty;
        public string cie_per { get; set; } = string.Empty;

        // 🔹 Factores decimales (nullable)
        public decimal? val_fac_ag { get; set; }
        public decimal? val_fac_cu { get; set; }
        public decimal? val_fac_pb { get; set; }
        public decimal? val_fac_zn { get; set; }
        public decimal? val_fac_au { get; set; }

        // 🔹 Factores presupuestados (nullable)
        public decimal? val_fac_bud_ag { get; set; }
        public decimal? val_fac_bud_cu { get; set; }
        public decimal? val_fac_bud_pb { get; set; }
        public decimal? val_fac_bud_zn { get; set; }
        public decimal? val_fac_bud_au { get; set; }

        // 🔹 Consumos (nullable)
        public decimal? val_con_ag { get; set; }
        public decimal? val_con_cu { get; set; }
        public decimal? val_con_pb { get; set; }
        public decimal? val_con_zn { get; set; }
        public decimal? val_con_au { get; set; }

        // 🔹 Auditoría
        public string? usu_creo { get; set; }
        public DateTime? fec_creo { get; set; }
        public string? usu_modi { get; set; }
        public DateTime? fec_modi { get; set; }

    }
}
