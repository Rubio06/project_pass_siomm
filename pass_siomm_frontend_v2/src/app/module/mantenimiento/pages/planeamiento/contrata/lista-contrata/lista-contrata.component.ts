...
import { Contrata, GrupoControlMant, LaborFiltro, LaborMant, ListZonas, MaestrosLabor, Nivel, NivelMant, PaginacionLabor, ProcedenciaBalanzaMant, ResponseApi, ResponseEliminarDto, RutaTransporte, TipoLabor, TipoLaborMant, UnidadEconomicaMant, VetaMant } from 'src/app/module/mantenimiento/interfaces/manenimiento.interface';
...
@Component({
    selector: 'app-lista-contrata',
    imports: [CommonModule],
    templateUrl: './lista-contrata.component.html',
    styleUrl: './lista-contrata.component.css'
})
export class ListaContrataComponent {
    listContrata = input<Contrata[]>([])
    isLoading = input<boolean>(false);


    idNivelSeleccionada = input<string | null>(null);

    onContrataSeleccionada = output<Contrata>();

    onEliminarRutaTransporte = output<Contrata>();
    ...
    public seleccionarContrata(contrata: Contrata) {
        this.onContrataSeleccionada.emit(contrata);
    }

    public onEliminar(rutaTransporte: Contrata) {
